...
{if $block == 1}
	...
	{foreach from=$cms_titles key=cms_key item=cms_title}
		<div id="informations_block_left_{$cms_key}" class="block informations_block_left">
			<div class="title_block"><a href="{$cms_title.category_link|escape:'html'}">{if !empty($cms_title.name)}{$cms_title.name}{else}{$cms_title.category_name}{/if}</a></div>
			<ul class="block_content">
				...
				{/foreach}
				{if $cms_title.display_store}<li class="last_item"><a href="{$link->getPageLink('stores')|escape:'html'}" title="{l s='Our stores' mod='blockcms'}">{l s='Our stores' mod='blockcms'}</a></li>{/if}
			</ul>
		</div>
	{/foreach}
	<!-- /Block CMS module -->
	...
	<div class="block_various_links" id="block_various_links_footer">
		...
		<ul>
			...
			{foreach from=$cmslinks item=cmslink}
				...
				{/if}
			{/foreach}
			<li class="item"><a href="{$link->getPageLink('sitemap')|escape:'html'}" title="{l s='sitemap' mod='blockcms'}">{l s='Sitemap' mod='blockcms'}</a></li>
