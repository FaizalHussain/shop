	...
	<table id="order-list" class="std">
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				<td class="history_link bold" data-label="{l s='Order reference'}">
				...
				<td class="history_date bold" data-label="{l s='Date'}">{dateFormat date=$order.date_add full=0}</td>
				<td class="history_price" data-label="{l s='Total price'}"><span class="price">{displayPrice price=$order.total_paid currency=$order.id_currency no_utf8=false convert=false}</span></td>
				<td class="history_method" data-label="{l s='Payment: '}">{$order.payment|escape:'htmlall':'UTF-8'}</td>
				<td class="history_state" data-label="{l s='Status'}">{if isset($order.order_state)}{$order.order_state|escape:'htmlall':'UTF-8'}{/if}</td>
				<td class="history_invoice" data-label="{l s='Invoice'}">
				{if (isset($order.invoice) && $order.invoice && isset($order.invoice_number) && $order.invoice_number) && isset($invoiceAllowed) && $invoiceAllowed == true}
					<a href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order={$order.id_order}")|escape:'html'}" title="{l s='Invoice'}" class="_blank"><img src="{$img_dir}icon/pdf.png" alt="{l s='Invoice'}" class="icon" /></a>
...
<ul class="footer_links clearfix">
	<li><a href="{$link->getPageLink('my-account', true)|escape:'html'}"><img src="{$img_dir}icon/my-account.png" alt="" class="icon" /> {l s='Back to Your Account'}</a></li>
