...
<p>
	<img src="{$img_dir}icon/pdf.png" alt="" class="icon" />
...
{/if}
{if $order->recyclable}
<p><img src="{$img_dir}icon/recyclable.png" alt="" class="icon" />&nbsp;{l s='You have given permission to receive your order in recycled packaging.'}</p>
{/if}
{if $order->gift}
	...
	<table class="detail_step_by_step std">
		...
		{foreach from=$order_history item=state name="orderStates"}
			<tr class="{if $smarty.foreach.orderStates.first}first_item{elseif $smarty.foreach.orderStates.last}last_item{/if} {if $smarty.foreach.orderStates.index % 2}alternate_item{else}item{/if}">
				<td data-label="{l s='Date'}">{dateFormat date=$state.date_add full=1}</td>
				<td data-label="{l s='Status'}">{$state.ostate_name|escape:'htmlall':'UTF-8'}</td>
			</tr>
		{/foreach}
		</tbody>
	</table>
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						<td data-label="{l s='Reference'}"><label for="cb_{$product.id_order_detail|intval}">{if $product.product_reference}{$product.product_reference|escape:'htmlall':'UTF-8'}{else}--{/if}</label></td>
						<td class="bold" data-label="{l s='Product'}">
							<label for="cb_{$product.id_order_detail|intval}">{$product.product_name|escape:'htmlall':'UTF-8'}</label>
						</td>
						<td data-label="{l s='Quantity'}"><input class="order_qte_input"  name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$product.customizationQuantityTotal|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$product.customizationQuantityTotal|intval}</span></label></td>
						{if $order->hasProductReturned()}
							<td data-label="{l s='Returned'}">
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td data-label="{l s='Quantity'}"><input class="order_qte_input" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$productQuantity|intval}</span></label></td>
						{if $order->hasProductReturned()}
							<td data-label="{l s='Returned'}">>
								{$product['qty_returned']}
							</td>
...
{if $order->getShipping()|count > 0}
	<table class="std">
		...
		<tbody>
			...
			<tr class="item">
				<td data-label="{l s='Date'}">{dateFormat date=$line.date_add full=0}</td>
				<td data-label="{l s='Carrier'}">{$line.carrier_name}</td>
				<td data-label="{l s='Weight'}">{if $line.weight > 0}{$line.weight|string_format:"%.3f"} {Configuration::get('PS_WEIGHT_UNIT')}{else}-{/if}</td>
				<td data-label="{l s='Shipping cost'}">{if $order->getTaxCalculationMethod() == $smarty.const.PS_TAX_INC}{displayPrice price=$line.shipping_cost_tax_incl currency=$currency->id}{else}{displayPrice price=$line.shipping_cost_tax_excl currency=$currency->id}{/if}</td>
				<td data-label="{l s='Tracking number'}">
		...
		<table class="detail_step_by_step std">
			...
			{foreach from=$messages item=message name="messageList"}
				<tr class="{if $smarty.foreach.messageList.first}first_item{elseif $smarty.foreach.messageList.last}last_item{/if} {if $smarty.foreach.messageList.index % 2}alternate_item{else}item{/if}">
					...
					<td data-label="{l s='Message'}">{$message.message|escape:'htmlall':'UTF-8'|nl2br}</td>
				</tr>
			{/foreach}
			</tbody>
		</table>
