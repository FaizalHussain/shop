...
<span itemprop="category">{include file="$tpl_dir./breadcrumb.tpl"}</span>
<div id="primary_block" class="clearfix">

    <meta itemprop="description" content="{$product->description_short|strip_tags}" />
    <meta itemprop="type" content="website" />
    <meta itemprop="url" content="http://{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}" />
    <meta itemprop="availability" content="{if $product->quantity <= 0}{if $allow_oosp}{$product->available_later}{else}{l s='This product is no longer in stock'}{/if}{else}{$product->available_now}{/if}" />
    <meta itemprop="price" content="{if $priceDisplay >= 0 && $priceDisplay <= 2}{convertPrice price=$productPrice}{/if}" />
    <meta itemprop="pricecurrency" content="{$currencySign|html_entity_decode:2:"UTF-8"}" />
	...
	<div id="pb-right-column">
		...
		{/if}
		{if isset($images) && count($images) > 1}<p class="resetimg clear"><span id="wrapResetImages" style="display: none;"><img src="{$img_dir}icon/displayall.png" alt="{l s='Cancel'}" width="11" height="13"/> <a id="resetImages" href="{$link->getProductLink($product)|escape:'html'}" onclick="$('span#wrapResetImages').hide('slow');return (false);">{l s='Display all pictures'}</a></span></p>{/if}
		<!-- usefull links-->
						...
						{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<li class="ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
								...
								<p class="product_accessories_price clearfix">
									<a class="lnk_more" href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{l s='View'}">{l s='View'}</a>
